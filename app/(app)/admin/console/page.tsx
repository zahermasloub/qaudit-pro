...
export default async function AdminConsolePage() {
  ...
  return (
    <div className="space-y-6 text-contrast">
      <section className="rounded-2xl border border-stroke/80 bg-surface shadow-soft">
        <div className="px-6 pt-6">
          <h2 className="text-xl font-semibold md:text-2xl">لوحة التحكم الموحّدة</h2>
        ...
        <div className="px-4 pb-4 md:px-6">
          <Tabs defaultValue="overview" className="mt-4">
            <TabsList className="flex flex-wrap gap-2 rounded-full bg-muted p-1 dark:bg-surface/20">
              {TABS.map(tab => (
                <TabsTrigger
                  key={tab.key}
                  value={tab.key}
                  className="rounded-full px-4 py-2 text-sm transition hover:bg-surface/70 data-[state=active]:bg-surface data-[state=active]:text-brand-700 data-[state=active]:shadow-soft dark:data-[state=active]:text-brand-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"
                >
                  {tab.label}
                </TabsTrigger>
              ))}
