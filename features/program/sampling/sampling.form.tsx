...
import { samplingSchema, type SamplingFormValues } from '@/features/program/sampling/sampling.schema';
...
export default function SamplingForm({
  ...
  const { register, handleSubmit, reset, watch, formState: { errors } } = useForm<SamplingFormValues>({
  ...
  function getMethodLabel(method: string): string {
    switch (method) {
      case 'random': return 'عشوائية';
      case 'judgment': return 'حكمية';
      case 'monetary': return 'وحدة نقدية';
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                max={populationSize}
              />
              {errors.sampleSize && <p className="text-red-500 text-xs mt-1">{errors.sampleSize.message}</p>}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="text-sm text-blue-800 space-y-1">
              ...
              <p>• حجم العينة: {sampleSize} من أصل {populationSize}</p>
              <p>• النسبة: {calculateSampleRatio()}</p>
              {method === 'monetary' && (
                <p>• سيتم إنشاء hash SHA256 لضمان سلامة العينة</p>
              )}
            </div>
          </div>
