...
SELECT e.engagement_id,e.audit_id,e.dept_id,e.team_id,e.status,
...
SELECT r.rec_id, f.finding_id, e.engagement_id, r.owner_dept_id, r.priority, r.status, r.due_date,
  ...
  CASE WHEN COUNT(a.action_id)=0 THEN NULL ELSE ROUND(SUM(COALESCE(a.progress_pct,0))::numeric/COUNT(a.action_id),1) END AS avg_progress
